...
public class F1CoreCalculation {
	public double x,y,z;
	...
	public void assignF1Variable(String StDate)
	{
		...
		try
		{
	        ...
	        ds.setURL("jdbc:h2:"+dbName);
	         ...
	         stmt.execute("SELECT X,Y,Z,T1,T2,T4 FROM TBL_FORMULA where fname ='FORMULA 1'");
	...
	public void assignF1TradeInfo(String scripcode)
	{
		...
		try
		{
			if (scripcode !=null)
			{
		        ...
		        ds.setURL("jdbc:h2:"+dbName);
		         conn = ds.getConnection(USER,PASS);
		         stmt = conn.createStatement();
		         stmt.execute("Select * from TBL_F1_TRADEINFO where symbol = '"+scripcode+"'");
		         ResultSet rs =stmt.getResultSet(); 
		         rs.next();
		         if (rs !=null)
		         {
							...
							if ((low == 0.0) && (high == 0.0))
					    	    {
					    	    		...
					    	    		executequery("update TBL_F1_TRADEINFO  set low="+low+", high="+high+" where symbol='"+instrument+"'");
					    	    }
							if ((isBought==false)&&(isSell==false))
							{
						    	    if ( tickprice > high) {high = tickprice;executequery("update TBL_F1_TRADEINFO  set high="+high+" where symbol='"+instrument+"'");}
						    	    if ( tickprice < low) {low = tickprice;executequery("update TBL_F1_TRADEINFO  set low="+low+" where symbol='"+instrument+"'");}
							}
					    	    if (isBought == true)
					    	    {
					    	    		if (tickprice > Mpoint + (Mpoint*(y/100)))
					    	    		{
					    	    			...
					    	    			TCount =TCount +1; 
					    	    			executequery("update TBL_F1_TRADEINFO set EXITCONDITION='PROFIT',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    		...
					    	    		else if(tickprice < Mpoint - (Mpoint*(z/100)))
					    	    		{
					    	    			...
					    	    			TCount =TCount +1;
					    	    			executequery("update TBL_F1_TRADEINFO set EXITCONDITION='LOSS',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    			calculatefigure(strsymbol);
					    	    			break;
					    	    		}//ticktime > t4
					    	    		...
					    	    		{				    	    			
					    	    			...
					    	    			TCount =TCount +1;
					    	    			executequery("update TBL_F1_TRADEINFO set EXITCONDITION='Validity Expires',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    			calculatefigure(strsymbol);
					    	    			break;	
					    	    		}
					    	    ...
					    	    else if (isSell == true)
					    	    {
					        	    	if (tickprice < Mpoint - (Mpoint*(y/100)))
					    	    		{
					        	    		...
					        	    		executequery("update TBL_F1_TRADEINFO set EXITCONDITION='PROFIT',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
					    	    		else if(tickprice > Mpoint + (Mpoint*(z/100)))
					    	    		{
					        	    		...
					        	    		TCount =TCount +1;
					        	    		executequery("update TBL_F1_TRADEINFO set EXITCONDITION='LOSS',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
					    	    		else if(ticktime.after(t4))
					    	    		{
					        	    		...
					        	    		TCount =TCount +1;
					        	    		executequery("update TBL_F1_TRADEINFO set EXITCONDITION='Validity Expires',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
	...
	public void InsertPL(String sym, double pl, double percent, int tcount)
	{
		...
		try
		{
	        ...
	        ds.setURL("jdbc:h2:"+dbName);
	         conn = ds.getConnection(USER,PASS);
	         stmt = conn.createStatement();
	         stmt.executeUpdate("update TBL_PLDATATABLE  set  F1PL="+pl+" , F1PRECENT = "+percent+" , F1TRADECOUNT = "+tcount+" where symbol ='"+sym+"'");
