...
public class F5CoreCalculation {
	...
	public void assignF5Variable(String StDate)
	{
		...
		try
		{
	        ...
	        ds.setURL("jdbc:h2:"+dbName);
	         ...
	         stmt.execute("SELECT X,Y,Z,T1,T2,T4 FROM TBL_FORMULA where fname ='FORMULA 5'");
	...
	public void assignF5TradeInfo(String scripcode)
	{
		...
		try
		{
			if (scripcode !=null)
			{
		        ...
		        ds.setURL("jdbc:h2:"+dbName);
		         conn = ds.getConnection(USER,PASS);
		         stmt = conn.createStatement();
		         stmt.execute("Select * from TBL_F5_TRADEINFO where symbol = '"+scripcode+"'");
		         ResultSet rs =stmt.getResultSet(); 
		         rs.next();
		         if (rs !=null)
		         {
		        	 	...
		        	 	if (rs.getString("Bpoint") != null) {Bpoint =Double.valueOf(rs.getDouble("Bpoint"));};
	...
	{	
		assignF5Variable(dateStartTime.split(" ")[0]);
		assignF5TradeInfo(strsymbol);
		try
		{
			if (TCount <2)
			{
				...
				for (int k=1; k<tradeData.length; k++)
				{
					...
					{
					    	    if ((low == 0.0) && (high == 0.0))//if (iteration==1)
					    	    {
					    	    		low = tickprice;
					    	    		high =tickprice;
					    	    		iteration = iteration+1;
					    	    		executequery("update TBL_F5_TRADEINFO  set low="+low+", high="+high+" where symbol='"+instrument+"'");
					    	    }
					    	    if ((isBought==false)&&(isSell==false))
							{
						    	    if ( tickprice > high) {high = tickprice;executequery("update TBL_F5_TRADEINFO  set high="+high+" where symbol='"+instrument+"'");}
						    	    if ( tickprice < low) {low = tickprice;executequery("update TBL_F5_TRADEINFO  set low="+low+" where symbol='"+instrument+"'");}
							}
					    	    if (isBought == true)
					    	    {
					    	    		if (tickprice > Mpoint + (Mpoint*(y/100)))
					    	    		{
					    	    			...
					    	    			TCount =TCount +1; 
					    	    			executequery("update TBL_F5_TRADEINFO set EXITCONDITION='PROFIT',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    			calculatefigure(strsymbol);
					    	    			break;
					    	    		}
					    	    		else if(tickprice < Bpoint)
					    	    		{
					    	    			//sell command
					    	    			sellPrice = Double.valueOf(tradeData[k+1][1]);
					    	    			fst2 = ticktime;
					    	    			...
					    	    			TCount =TCount +1;
					    	    			executequery("update TBL_F5_TRADEINFO set EXITCONDITION='LOSS',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    			calculatefigure(strsymbol);
					    	    			break;
					    	    		}//ticktime > t4
					    	    		...
					    	    		{				    	    			
					    	    			...
					    	    			TCount =TCount +1;
					    	    			executequery("update TBL_F5_TRADEINFO set EXITCONDITION='Validity Expires',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					    	    			calculatefigure(strsymbol);
					    	    			break;	
					    	    		}
					    	    ...
					    	    else if (isSell == true)
					    	    {
					        	    	if (tickprice < Mpoint - (Mpoint*(y/100)))
					    	    		{
					        	    		...
					        	    		executequery("update TBL_F5_TRADEINFO set EXITCONDITION='PROFIT',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
					    	    		else if(tickprice > Bpoint)
					    	    		{
					        	    		...
					        	    		TCount =TCount +1;
					        	    		executequery("update TBL_F5_TRADEINFO set EXITCONDITION='LOSS',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
					    	    		else if(ticktime.after(t4))
					    	    		{
					        	    		...
					        	    		TCount =TCount +1;
					        	    		executequery("update TBL_F5_TRADEINFO set EXITCONDITION='Validity Expires',EXITTIME='"+ticktime.toString()+"',BUYPRICE="+buyPrice+",Tcount="+TCount+" where symbol='"+instrument+"'");
					        	    		calculatefigure(strsymbol);
					        	    		break;
					    	    		}
					        	    ...
					        	    double buylegprice = low + (low*(x/100)), sellegprice = high - (high*(x/100));
						        	    ...
						        	    if (tickprice > buylegprice)
						        	    {
						        	    		...
						        	    		if (fst1.after(t2))
						        	    		{
						        	    			...
						        	    			TCount=1;
						        	    			executequery("update TBL_F5_TRADEINFO  set ISSHOTSELL='"+isShotsell+"', ENTRYTIME = '"+ticktime.toString()+"',BUYPRICE="+buyPrice+", Mpoint="+Mpoint+", Bpoint="+Bpoint+" ,isBought='"+isBought+"',Tcount="+TCount+" where symbol='"+instrument+"'");
						        	    ...
						        	    else if (tickprice < sellegprice)
						        	    {
						        	    		...
						        	    		if (fst1.after(t2))
						        	    		{
						        	    			...
						        	    			System.out.println("HIGH : "+high+", LOW : "+low);
						        	    			executequery("update TBL_F5_TRADEINFO set ISSHOTSELL='"+isShotsell+"', ENTRYTIME='"+ticktime.toString()+"',SELLPRICE="+sellPrice+",Mpoint="+Mpoint+", Bpoint="+Bpoint+", isSell='"+isSell+"',Tcount="+TCount+"  where symbol='"+instrument+"'");
	...
	public void InsertPL(String sym, double pl, double percent, int tcount)
	{
		...
		try
		{
	        ...
	        ds.setURL("jdbc:h2:"+dbName);
	         conn = ds.getConnection(USER,PASS);
	         stmt = conn.createStatement();
	         stmt.executeUpdate("update TBL_PLDATATABLE  set  F5PL="+pl+" , F5PRECENT = "+percent+" , F5TRADECOUNT = "+tcount+" where symbol ='"+sym+"'");
